...
function PadCard({ pad }: { pad: Mousepad }) {
    const colorway = getColorwayBySlug(pad, getFeaturedColorwaySlug(pad));
    const title = `${pad.brand} ${pad.name}`;

    return (
        <Card
            className="overflow-hidden border-border bg-card/95 shadow-lg shadow-black/10"
            style={{
            ...
            <div className="space-y-5 p-5">
                ...
                <div className="flex flex-wrap gap-2">
                    ...
                    <Badge variant="secondary">
                        {formatMousepadValue(pad.softness)}
                    </Badge>
                    <Badge variant="outline">
                        {formatMousepadValue(pad.surface)}
                    </Badge>
                    <Badge variant="outline">
                        {formatMousepadValue(pad.base)}
                    </Badge>
                </div>
                    ...
                    <div className="border-t border-border/60 pt-4 space-y-2">
                        ...
                        {pad.communityConsensus.commonComparisons?.length ? (
                            <div className="flex flex-wrap gap-1.5 pt-1">
                                {pad.communityConsensus.commonComparisons
                                    .slice(0, 3)
                                    .map((c) => (
                                        <Badge
                                            key={c}
