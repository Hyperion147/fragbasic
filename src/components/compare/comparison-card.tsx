    ...
    <Link href={`/compare/${comparison.slug}`} className="h-full">
      <Card className="group h-full border-border bg-card/95 p-5 transition-all duration-200 hover:-translate-y-0.5 hover:border-primary/45 hover:shadow-lg hover:shadow-black/10">
        <div className="flex items-start justify-between gap-3">
          ...
          <Badge variant={comparison.status === "published" ? "default" : "outline"}>
            {comparison.status}
          </Badge>
        </div>

        <div className="grid gap-3 rounded-3xl border border-border bg-background/70 p-4 md:grid-cols-[1fr_auto_1fr] md:items-center">
          <PadSummary
            brand={left.brand}
            name={left.name}
