    ...
    <main className="min-h-screen bg-background text-foreground">
      <section className="border-b border-border">
        <div className="mx-auto max-w-7xl px-4 py-14 md:px-8 md:py-20">
          <p className="text-sm text-muted-foreground">Mousepad database</p>
          ...
          <p className="mt-5 max-w-2xl text-base leading-7 text-muted-foreground">
            Open any mousepad profile to view feel ratings, specs, availability,
            and competitive FPS recommendations.
          </p>
        </div>
      </section>
            ...
            <Link key={pad.slug} href={`/mousepads/${pad.slug}`}>
              <Card className="group h-full border-border bg-card p-5 transition-colors hover:border-primary/50">
                <div className="mb-4 flex flex-wrap gap-2">
                  <Badge variant="secondary">{formatValue(pad.category)}</Badge>
