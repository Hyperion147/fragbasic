...
export async function POST(request: Request) {
  ...
  const glasspadId = form?.get("glasspadId");
  if (!(file instanceof File) || typeof glasspadId !== "string" || !glasspadId.trim()) return jsonError("glasspadId and an image file are required", 400);
  ...
  const objectPath = `glasspads/${glasspadId.trim()}/${crypto.randomUUID()}.${extension}`;
